...
data <- read.csv(paste(p.rawdata, "MyDataSheet.csv", sep = ""), strip.white = TRUE, stringsAsFactors = FALSE)
...
#shortening the column name for participation since it is way long
colnames(d)[3] <- "Participation.pretty"
...
# Additional functions needed to run ^
